...
class IgdbPlatformConverterTest {
    ...
    fun `toGamePlatform should convert platforms with minimal fields`() {
        ...
        result shouldBeEqual FullObject(GamePlatform.Other("Meta Quest 3"))
    }

    @Test
    fun `toGamePlatform should fail when required fields are not defined`() {
        shouldThrow<IllegalArgumentException> {
            IgdbPlatform(id = 471, name = "Meta Quest 3", slug = "").toGamePlatformRef()
        }
        shouldThrow<IllegalArgumentException> {
            IgdbPlatform(id = 471, name = "", slug = "meta-quest-3").toGamePlatformRef()
        }
    }
    ...
    internal companion object {
        ...
        fun platformConverterTestSource(): List<Pair<IgdbPlatform, Ref<GamePlatform>>> {
            ...
            val refs = fullObjects.map { (igdbPlatform, gamePlatform) ->
                IgdbPlatform(id = igdbPlatform.id) to gamePlatform
            }

            val slugs = fullObjects.map { (igdbPlatform, gamePlatform) ->
                IgdbPlatform(id = igdbPlatform.id + 1000, slug = igdbPlatform.slug) to gamePlatform
            }

            return fullObjects + refs + slugs + listOf(
                IgdbPlatformFixtures.commodoreC64 to FullObject(GamePlatform.Other("Commodore C64/128/MAX")),
                IgdbPlatform(id = 10000) to Ref.Id(IgdbGamePlatformId(10000)),
            )
        }
    }
}
