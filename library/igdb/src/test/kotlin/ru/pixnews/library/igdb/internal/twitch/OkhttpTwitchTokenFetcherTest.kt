...
class OkhttpTwitchTokenFetcherTest {
    ...
    fun `Fetcher should return correct error on HTTP 403, incorrect client secret`() = coroutinesExt.runTest {
        ...
        response shouldBe instanceOf<HttpFailure<TwitchErrorResponse>>()

        val httpFailure: HttpFailure<TwitchErrorResponse> = response as HttpFailure<TwitchErrorResponse>
        httpFailure.httpCode shouldBe 403
        httpFailure.response shouldBe TwitchErrorResponse(
            status = 403,
    ...
    private fun startServerPrepareApi(
        okhttpClient: OkHttpClient = createOkHttpClientBuilder().build(),
