...
class IgdbCompanyConverterTest {
    ...
    fun `IgdbCompanyConverter should convert companies`(testData: Pair<IgdbCompany, Ref<Company>>) {
        val result = IgdbCompanyConverter.convertToCompanyRef(testData.first)
        result shouldBeEqual testData.second
    }

    @Test
    fun `IgdbCompanyConverter should fail when required fields are not requested`() {
        shouldThrow<IllegalArgumentException> {
            IgdbCompanyConverter.convertToCompanyRef(IgdbCompany(id = 0, name = "Larian Studios"))
        }
    }
    ...
    companion object {
        @JvmStatic
        @Suppress("LongMethod")
        fun companyTestSource(): List<Pair<IgdbCompany, Ref<ru.pixnews.domain.model.company.Company>>> = listOf(
            ...
            IgdbCompany(id = 510) to Id<ru.pixnews.domain.model.company.Company, CompanyId>(
                IgdbCompanyId(id = 510),
            ),
        )
    }
}
